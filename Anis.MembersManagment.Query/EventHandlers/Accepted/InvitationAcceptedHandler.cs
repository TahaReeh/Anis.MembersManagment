...
namespace Anis.MembersManagment.Query.EventHandlers.Accepted
{
    public class InvitationAcceptedHandler : IRequestHandler<InvitationAccepted, bool>
    {
        ...
        public async Task<bool> Handle(InvitationAccepted @event, CancellationToken cancellationToken)
        {
            ...
            {
                await _unitOfWork.Subscriber.ChangeStatusAsync(Subscriber.FromInvitationAcceptedEvent(@event));
            }
            else
            {
            ...
            var permission = await _unitOfWork.Permission.GetAsync(p =>
            p.UserId == @event.Data.MemberId && p.SubscriptionId == @event.Data.SubscriptionId);

            if (permission is not null)
            {
                await _unitOfWork.Permission.UpdateSequence(
                    @event.Data.MemberId,@event.Data.SubscriptionId,@event.Sequence
                    );
            }
